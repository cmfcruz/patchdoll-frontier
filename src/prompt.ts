...
export function buildAgentPrompt(input: PromptInput): string {
  return [
    `You are ${agent.displayName} running as Ember for a Slack request.`,
    "Reply with a concise, Slack-ready answer.",
    "If you change files, include the changed paths and any checks you ran.",
    "Ember is configured by environment variables only; it has no runtime settings to change.",
    githubConfigured()
      ? "Before committing or pushing to github.com, call the Ember MCP tool `ember_enable_github` once; it sets the bot's git identity and credentials so normal git commands work."
      : undefined,
    "",
