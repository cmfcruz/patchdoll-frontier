...
// The flow:
//
//   1. The agent calls the `ember_enable_github` MCP tool (see mcp.ts), which
...
const helperPath = join(emberHome, ".ember", "git-credential-ember.cjs");
...
"use strict";
// Generated by the Ember bridge. Fetches a GitHub credential on demand.
...
function gitConfig(key: string, value: string): Promise<void> {
  return new Promise((resolveConfig, reject) => {
    const child = spawn("git", ["config", "--global", key, value], {
      env: { ...process.env, HOME: emberHome },
      stdio: "ignore"
    });
